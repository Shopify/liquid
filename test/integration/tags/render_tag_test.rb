...
class RenderTagTest < Minitest::Test
  ...
  def test_render_with_no_arguments
    Liquid5::Template.file_system = StubFileSystem.new('source' => 'rendered content')
  ...
  def test_render_passes_named_arguments_into_inner_scope
    Liquid5::Template.file_system = StubFileSystem.new('product' => '{{ inner_product.title }}')
  ...
  def test_recursively_rendered_template_does_not_produce_endless_loop
    Liquid5::Template.file_system = StubFileSystem.new('loop' => '{% render "loop" %}')

    assert_raises(Liquid5::StackLevelError) do
  ...
  def test_dynamically_choosen_templates_are_not_allowed
    Liquid5::Template.file_system = StubFileSystem.new('snippet' => 'should not be rendered')

    assert_raises(Liquid5::SyntaxError) do
      Liquid5::Template.parse("{% assign name = 'snippet' %}{% render name %}")
    end
  end
  ...
  def test_render_tag_within_if_statement
    Liquid5::Template.file_system = StubFileSystem.new('snippet' => 'my message')
    assert_template_result('my message', '{% if true %}{% render "snippet" %}{% endif %}')
  end

  def test_break_through_render
    Liquid5::Template.file_system = StubFileSystem.new('break' => '{% break %}')
  ...
  def test_increment_is_isolated_between_renders
    Liquid5::Template.file_system = StubFileSystem.new('incr' => '{% increment %}')
    assert_template_result('010', '{% increment %}{% increment %}{% render "incr" %}')
  end

  def test_decrement_is_isolated_between_renders
    Liquid5::Template.file_system = StubFileSystem.new('decr' => '{% decrement %}')
  ...
  def test_includes_will_not_render_inside_render_tag
    ...
    exc = assert_raises(Liquid5::DisabledError) do
      Liquid5::Template.parse('{% render "test_include" %}').render!
    end
  ...
  def test_includes_will_not_render_inside_nested_sibling_tags
    ...
    output = Liquid5::Template.parse('{% render "nested_render_with_sibling_include" %}').render
