...
class TemplateTest < Minitest::Test
  ...
  def test_resource_limits_hash_in_template_gets_updated_even_if_no_limits_are_set
    ...
    t.render!
    assert t.resource_limits.assign_score.positive?
    assert t.resource_limits.render_score.positive?
    assert t.resource_limits.render_length.positive?
  end
  ...
  def test_default_resource_limits_unaffected_by_render_with_context
    ...
    t = Template.parse('{% for a in (1..100) %} {% assign foo = 1 %} {% endfor %}')
    t.render!(context)
    assert context.resource_limits.assign_score.positive?
    assert context.resource_limits.render_score.positive?
    assert context.resource_limits.render_length.positive?
  end
  ...
  def test_exception_renderer_that_raises
    ...
    assert_raises(Liquid::ZeroDivisionError) do
      Template.parse('{{ 1 | divided_by: 0 }}').render({}, exception_renderer: lambda { |e|
        exception = e
        raise
      })
    end
